<template lang="pug">
  #app
    nav.ui.fixed.top.labeled.icon.menu
      router-link.item(v-tap to="/")
        i.home.icon
        span.big 認識
      router-link.item(v-tap to="/about")
        i.exchange.icon
        span.big 猜猜看
      router-link.item(v-tap to="/test")
        i.question.icon
        span.big 自我測驗 
    router-view(:qs = "qs")
</template>
...
export default {
  ...
  data () {
    return {
      qs: [
        {
          t: '回想一下，你是如何學習新事物的，試著避開選擇像肢體類的技術學習，如騎腳踏車。你比較偏好從：',
          cs: ['文字解說，如手冊或是教科書。', '觀看現場的示範', '圖片或是圖表－視覺上的提示', '別人解釋或是問問題'],
          checked: [false, false, false, false],
          rs: ['r', 'k', 'v', 'a']
        }, {
          t: '你想學習一個新的程式語言、電腦技術或是電玩，你會：',
          ...
          t: '你不確定一個字要拼成 dependent 還是 dependant時，你會：',
          cs: ['把兩個字都寫在紙上，選一個', '在腦中「看」這個字，並且從他們看起來的外觀來選', '查字典', '想想這兩個字的發音，並且選擇一個'],
          checked: [false, false, false, false],
          rs: ['k', 'v', 'r', 'a']
        }, {
          t: '你完成了某項測試，你會喜歡那種方式呈現結果？',
...
nav a.item.router-link-exact-active {
